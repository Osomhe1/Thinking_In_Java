package com.mycompany.generices;
import java.util.*;
...
public class File2{
   ...
   private static interface Addable<T> {
    ...
    public static void main(String[] args) {
        ForNameCreator petsCreator;
       petsCreator = new ForNameCreator();
        int productSize = 3;
        List<Pet> petsList = petsCreator.arrayList(productSize);
        Filler.fill(new AddableCollection<Pet>(petsList), Pet.class, productSize);
        Filler.fill(Adapter.addableCollection(petsList), new Generator<Pet>() {
            public Pet next() {
